...
export class AnthropicProvider implements ILLMProvider {
  ...
  constructor(apiKey = process.env.ANTHROPIC_API_KEY || "sk-ant-dummy-key") {
    this.apiKey = apiKey;
  }

  async generateCompletion(options: LLMCompletionOptions): Promise<LLMCompletionResponse> {
    const startTime = Date.now();
    const content = `[Claude 3.5 Anthropic Output]\nResponding to: "${options.userPrompt}"`;
    const latencyMs = Date.now() - startTime + Math.floor(Math.random() * 250) + 180;
